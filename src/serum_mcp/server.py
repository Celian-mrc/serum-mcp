...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- oscillators[].granular_source (indices 0-2, experimental -- decoded from real "
        "files 2026-07-30, not yet confirmed live) uses Serum's GRANULAR engine "
        "(GranularOsc): an absolute path to a WAV file, continuously re-triggered as "
        "short randomizable 'grains' instead of played back straight -- for pads/"
        "textures/soundscapes built FROM a sample (a field recording, a vocal, a drone, "
        "an ambient texture) rather than the sample staying recognizable as itself (use "
        "sample_playback_source for that instead). granular_density (Hz, higher = denser/"
        "smoother cloud), granular_grain_length (seconds), granular_random_pitch/pan/"
        "grain_length add organic variation between grains -- 0 on all three gives a "
        "robotic/uniform-sounding cloud, real presets almost always randomize at least "
        "pan and grain length. warp_amount/warp_mode also apply (shared with "
        "sample_playback_source/wavetable). Takes priority over wavetable/"
        "custom_harmonics/sample_source, but sample_playback_source takes priority over "
        "this if both are set on the same oscillator. Only .wav is supported. Given the "
        "experimental status, mention that to the user and prefer test-in-serum before "
        "declaring a granular preset done.\n"
        "- oscillators[].spectral_source (indices 0-2, experimental -- same caveats as "
        "granular_source) uses Serum's SPECTRAL engine (SpectralOsc): an absolute path "
        ...
        "ties rate to tempo instead of free Hz. lfos[].mono (found live 2026-07-29) "
