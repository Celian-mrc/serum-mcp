...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "- arp turns on Serum's arpeggiator (algorithmic pattern modes only -- see "
        "ArpSpec for the full field list and shape options). Leave it UNSET (the "
        "PresetSpec default) for anything that isn't explicitly meant to arpeggiate; "
        "don't set it just because a role sounds rhythmic. Only use for requests like "
        "'make this arpeggiate', 'add an up/down arp', 'this should play as a chord "
        "arp' -- and mention up front that shape='pattern' (a fully custom hand-drawn "
        "note sequence) isn't supported yet, only the algorithmic shapes are.\n"
        ...
        "conflate the two. See docs/SOUND_DESIGN_REFERENCE.md for concrete per-role "
        "starting points (envelope shape, filter type/resonance, dominant warp_mode, "
        "typical mod_routes) derived the same way -- e.g. a bass role is almost always "
        "mono with a ~4ms attack/~45ms release and low filter resonance, a pluck role's "
        "defining trait is zero sustain (a real decaying pluck, not a held note), a pad "
        "role has a long (~600ms+) attack, FXComp+FXEQ anchor nearly every role's FX "
        "chain ahead of reverb/delay, and modulation is often macro-driven (performance-"
        "mappable) rather than purely LFO-driven except in pad/chord/arp roles. Consult "
        "it before generating a preset whose role matches one of its categories (bass, "
        "pluck, lead, pad, chords, synth, arp, sequence) instead of guessing envelope/"
        "filter/mod-route starting values from scratch.\n"
