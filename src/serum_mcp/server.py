...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "obvious ones; use `aux_source` for player-controllable modulation depth; use "
        "`arp` when a rhythmic/chopped character fits, not just for literal "
        "'arpeggiate my chord' requests; reach for `custom_harmonics` when a genuinely "
        "bespoke spectrum beats a curated table. None of this means pad EVERY preset "
        "with every feature regardless of fit (a simple role can still have a simple, "
        "well-executed answer) -- it means the DEFAULT effort level should assume the "
        "user is asking for what a skilled sound designer would spend real time on, not "
        "the fastest valid PresetSpec that satisfies the request. If a user's request "
