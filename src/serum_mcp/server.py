...
mcp = FastMCP(
    ...
    instructions=(
        ...
        "list_parameters()['multisample_instruments'] -- currently 'choir_ah', "
        "'synth_sid', 'guitar_ac', 'violins') uses Serum's MULTISAMPLE engine "
        "(MultiSampleOsc) with a CURATED real Factory multisample instrument -- for "
        "requests wanting a REALISTIC sampled instrument (choir, guitar, strings, ...) "
        "rather than a synthesized/textural source. Unlike sample_playback_source (one "
        "recording played across the whole keyboard), this plays the correct sample per "
        "note/keyzone with proper pitch, exactly as Xfer's own sound designers configured "
        "it -- confirmed live 2026-07-31 (rendering the same instrument an octave apart "
        "produced pitches exactly an octave apart, confirming real per-note sample "
        "selection). Only these 4 pre-verified instruments are selectable (arbitrary user "
        "sample files aren't supported for this engine -- use sample_playback_source or "
