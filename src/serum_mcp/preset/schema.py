...
class MultiSampleInstrumentDef:
    ...
    `files`' own keys use a literal DOUBLE BACKSLASH as path separator
    (confirmed by inspecting individual characters, not just `repr()` --
    e.g. ``"Choir Samples\\\\XFChoir Ah High A#2.flac"``, matching a JUCE/
    Windows path serialization quirk, not a transcription artifact) --
    preserve this exactly if ever hand-editing these entries.
    """
...
LFO_PARAMS: dict[str, ParamDef] = {
    "kParamRate": ParamDef(
        ...
        "independently confirmed. **The anomaly above raw~35-40 is UNCHANGED and "
        "confirmed real, independent of the beat_sync bug** (this re-run used the fixed "
        "code and still hit it): raw 40/60/80 all measured the identical 21.6Hz, while "
        "50/70/90/100 gave inconsistent, non-monotonic values (11.6/18.4/8.4/23.2Hz) -- "
        "genuinely undetermined whether this is real Serum behavior or a "
        "render/measurement-pipeline artifact specific to fast control-rate modulation; "
        "needs a live Serum GUI cross-check to disambiguate. See "
        "docs/PARAMETER_SCHEMA.md item 6a for the full retraction writeup and "
        "reference-serum-verify-audio-pipeline for the pipeline-side notes.",
    ),
    "kParamMode": ParamDef(
        "kParamMode",
        "enum",
